...
alp=np.arange(0.01,0.21,0.01)
peak=np.empty(20)
sumkekka=np.empty(20)
# （3）SIRモデル
# 3-1パラメータ
...
sum1[0]=0# （4）数値積分 4次ルンゲ-クッタ法 4th-Order Runge–Kutta Methods
for i in range(20):
  alpha1=alp[i]
  S1[0] = S_0
  Im1[0] = Im_0
...
plt.plot(alp, peak, color = "blue", label = "alpha=0.01 max={}".format(max(Is1)), linewidth = 1.0)

#plt.plot(t, R1, color= "blue", label = "R:免疫獲得者", linewidth = 1.0)
# グラフの見た目設定
plt.title('alphaを変えた時のピークの変化')  # グラフタイトル パラメータmとTの値表示
...
plt.plot(alp, sumkekka, color = "blue", label = "alpha=0.01 sum={}".format(sum1[n-1]), linewidth = 1.0)

#plt.plot(t, R1, color= "blue", label = "R:免疫獲得者", linewidth = 1.0)
# グラフの見た目設定
plt.title('alphaを変えた時の累積')  # グラフタイトル パラメータmとTの値表示
plt.yticks(np.arange(0,N+0.1,N/10))    # y軸 目盛りの配分 0からN（=1000万）までを10等分 N/10（=100万）刻み Nを含めるためNをN+0.1としておく
plt.gca().set_yticklabels(['{:.0f}%'.format(y/((N)/100)) for y in plt.gca().get_yticks()])   # y軸目盛りを％表示に変更
plt.xlabel('alpha')                              # 横軸ラベル
plt.ylabel('人数')      # 縦軸ラベル
