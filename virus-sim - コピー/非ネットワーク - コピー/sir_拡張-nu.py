...
plt.plot(t, Is1, color = "blue", label = "nu=0. max={}".format(max(Is1)), linewidth = 1.0)
plt.plot(t, Is2, color = "red", label = "nu=0.01 max={}".format(max(Is2)), linewidth = 1.0)
plt.plot(t, Is3, color = "green", label = "nu=0.02 max={}".format(max(Is3)), linewidth = 1.0)
...
plt.show()
#累積のほうのグラフ
plt.plot(t, sum1, color = "blue", label = "nu=0 sum={}".format(sum1[n-1]), linewidth = 1.0)
plt.plot(t, sum2, color = "red", label = "nu=0.01 sum={}".format(sum2[n-1]), linewidth = 1.0)
plt.plot(t, sum3, color = "green", label = "nu=0.02 sum={}".format(sum3[n-1]), linewidth = 1.0)
plt.title('SIRモデル RK4によるシミュレーション')  # グラフタイトル パラメータmとTの値表示
plt.yticks(np.arange(0,N+0.1,N/10))    # y軸 目盛りの配分 0からN（=1000万）までを10等分 N/10（=100万）刻み Nを含めるためNをN+0.1としておく
plt.gca().set_yticklabels(['{:.0f}%'.format(y/((N)/100)) for y in plt.gca().get_yticks()])   # y軸目盛りを％表示に変更
plt.xlabel('時間（日）')                              # 横軸ラベル
