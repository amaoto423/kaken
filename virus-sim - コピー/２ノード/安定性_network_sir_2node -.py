...
# グラフの見た目設定
plt.title('ノード1の結果')  # グラフタイトル パラメータmとTの値表示
...
plt.plot(t, S2, color = "blue", label = "S:未感染者", linewidth = 1.0)
plt.plot(t, Im2, color = "red", label = "Im:軽症者", linewidth = 1.0)
plt.plot(t, Is2, color = "green", label = "Is:重症者", linewidth = 1.0)
plt.plot(t, R2, color = "black", label = "R:回復者", linewidth = 1.0)
#plt.plot(t, R1, color= "blue", label = "R:免疫獲得者", linewidth = 1.0)
# グラフの見た目設定
plt.title('ノード2の結果')  # グラフタイトル パラメータmとTの値表示
#plt.yticks(np.arange(0,N+0.1,N/10))    # y軸 目盛りの配分 0からN（=1000万）までを10等分 N/10（=100万）刻み Nを含めるためNをN+0.1としておく
#plt.gca().set_yticklabels(['{:.0f}%'.format(y/((N)/100)) for y in plt.gca().get_yticks()])   # y軸目盛りを％表示に変更
plt.xlabel('時間（日）')                              # 横軸ラベル
plt.ylabel('人数')      # 縦軸ラベル
