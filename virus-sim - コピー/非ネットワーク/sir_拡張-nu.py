...
plt.show()
#累積のほうのグラフ
plt.plot(t, sum1, color = "blue", label = "nu=0 sum={:.3f}".format(sum1[n-1]/N), linewidth = 1.0)
plt.plot(t, sum2, color = "red", label = "nu=0.01 sum={:.3f}".format(sum2[n-1]/N), linewidth = 1.0)
plt.plot(t, sum3, color = "green", label = "nu=0.02 sum={:.3f}".format(sum3[n-1]/N), linewidth = 1.0)
plt.title('小さい街:νを変更:時間変化')  # グラフタイトル パラメータmとTの値表示
plt.yticks(np.arange(0,N+0.1,N/10))    # y軸 目盛りの配分 0からN（=1000万）までを10等分 N/10（=100万）刻み Nを含めるためNをN+0.1としておく
plt.gca().set_yticklabels(['{:.0f}%'.format(y/((N)/100)) for y in plt.gca().get_yticks()])   # y軸目盛りを％表示に変更
plt.xlabel('時間（日）')                              # 横軸ラベル
plt.ylabel('人数（総人口に対する割合）')      # 縦軸ラベル
plt.grid(True)                                        # グリッド表示
plt.legend()          
plt.show()