...
n = 5*T                 # 変数tの範囲をn等分   n=T/h=T/0.1=10*T （T=250のときはn=2500）
h = 0.2                  # 等差数列の公差:0.1 固定
...
for j in range(n-1):     # j=0,...,n-2 -> S[j]=S[0],...,S[n-1]（I[j],R[j]も同じ） 要素数n個
  ...
  sum1[j+1]=sum1[j]+alpha1*h*Im1[j]
  

print(sum1[n-1])
plt.plot(t, S1, color = "green", label = "S:未感染者", linewidth = 1.0)
plt.plot(t, Im1, color = "red", label = "I:感染者", linewidth = 1.0)
plt.plot(t, Is1, color = "red", label = "I:感染者", linewidth = 1.0)
plt.plot(t, R1, color= "black", label = "R:免疫獲得者", linewidth = 1.0)
# グラフの見た目設定
