...
f=0 #flag
cnt=0
th=1000 #１日の感染者がこの値を超えたらlim日間ロックダウンする
lim=30 #ロックダウンの日数
# （3）SIRモデル
# 3-1パラメータ
...
plt.plot(lima, (peak[0]-peak)/(lima), color = "blue", label = "k=1", linewidth = 1.0)

#plt.plot(t, R1, color= "blue", label = "R:免疫獲得者", linewidth = 1.0)
# グラフの見た目設定
plt.title('封鎖期間の日数とピークの減衰の比の推移')  # グラフタイトル パラメータmとTの値表示
#plt.yticks(np.arange(0,N+0.1,N/10))    # y軸 目盛りの配分 0からN（=1000万）までを10等分 N/10（=100万）刻み Nを含めるためNをN+0.1としておく
#plt.gca().set_yticklabels(['{:.0f}%'.format(y/((N)/100)) for y in plt.gca().get_yticks()])   # y軸目盛りを％表示に変更
plt.xlabel('封鎖期間')                              # 横軸ラベル
plt.ylabel('減少量/コスト')      # 縦軸ラベル
plt.legend()                                          # 凡例表示

# 設定反映しプロット描画
plt.show()

plt.plot(lima, (sumkekka[0]-sumkekka)/(lima), color = "red", label = "k=1", linewidth = 1.0)
plt.title('封鎖期間の日数と累積の減衰の推移')  # グラフタイトル パラメータmとTの値表示
#plt.yticks(np.arange(0,N+0.1,N/10))    # y軸 目盛りの配分 0からN（=1000万）までを10等分 N/10（=100万）刻み Nを含めるためNをN+0.1としておく
#plt.gca().set_yticklabels(['{:.0f}%'.format(y/((N)/100)) for y in plt.gca().get_yticks()])   # y軸目盛りを％表示に変更
plt.xlabel('封鎖期間')                              # 横軸ラベル
plt.ylabel('減少量/コスト')      # 縦軸ラベル
...
plt.plot(lima, (sumkekka[0]-sumkekka)/(lima)**1.25, color = "red", label = "k=1.25", linewidth = 1.0)
plt.title('封鎖期間の日数と累積の減衰の推移')  # グラフタイトル パラメータmとTの値表示
#plt.yticks(np.arange(0,N+0.1,N/10))    # y軸 目盛りの配分 0からN（=1000万）までを10等分 N/10（=100万）刻み Nを含めるためNをN+0.1としておく
#plt.gca().set_yticklabels(['{:.0f}%'.format(y/((N)/100)) for y in plt.gca().get_yticks()])   # y軸目盛りを％表示に変更
plt.xlabel('封鎖期間')                              # 横軸ラベル
...
plt.plot(lima, (sumkekka[0]-sumkekka)/(lima)**1.5, color = "red", label = "k=1.5", linewidth = 1.0)
plt.title('封鎖期間の日数と累積の減衰の推移')  # グラフタイトル パラメータmとTの値表示
#plt.yticks(np.arange(0,N+0.1,N/10))    # y軸 目盛りの配分 0からN（=1000万）までを10等分 N/10（=100万）刻み Nを含めるためNをN+0.1としておく
#plt.gca().set_yticklabels(['{:.0f}%'.format(y/((N)/100)) for y in plt.gca().get_yticks()])   # y軸目盛りを％表示に変更
plt.xlabel('封鎖期間')                              # 横軸ラベル
...
plt.plot(lima, (sumkekka[0]-sumkekka)/(lima)**2, color = "red", label = "k=2", linewidth = 1.0)
plt.title('封鎖期間の日数と累積の減衰の推移')  # グラフタイトル パラメータmとTの値表示
#plt.yticks(np.arange(0,N+0.1,N/10))    # y軸 目盛りの配分 0からN（=1000万）までを10等分 N/10（=100万）刻み Nを含めるためNをN+0.1としておく
#plt.gca().set_yticklabels(['{:.0f}%'.format(y/((N)/100)) for y in plt.gca().get_yticks()])   # y軸目盛りを％表示に変更
plt.xlabel('封鎖期間')                              # 横軸ラベル
